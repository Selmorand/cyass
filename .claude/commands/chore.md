...
- IMPORTANT: You're writing a plan to resolve a chore based on the `Chore` that will add value to the CYAss application.
...
- Remember this is a PWA with Supabase backend - no traditional server code.
...
- `scripts/**` - Contains the batch scripts to setup, build, and run the client.
...
- `app/client/src/services/` - Supabase integration services
- `app/client/src/components/ItemInspection.tsx` - Mobile camera implementation (protected)
- `app/client/src/services/storage.ts` - Photo storage (protected)
- `app/client/src/services/pdf.tsx` - PDF generation with links (protected)
...
<Check /.claude/protected-code.md and list any protected code areas that relate to this chore. If the chore touches protected code, note that explicit permission is required.>
...
- `cd app/client && npm test` - Run any existing tests

## Notes
<optionally list any additional notes or context that are relevant to the chore that will be helpful to the developer>
- Remember: CYAss is a South African property inspection PWA
- Uses Supabase for backend (database, auth, storage)
- Must maintain mobile camera functionality
- Must preserve PDF generation with clickable image links
- All changes must respect Row Level Security (RLS) policies
```
